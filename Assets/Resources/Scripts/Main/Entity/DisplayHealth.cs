...
public class DisplayHealth : MonoBehaviour
{
    ...
    private void OnEnable()
    {
        ...
        if (gameObject.CompareTag("Ally"))
            entityHealthBar.transform.localPosition += new Vector3(0f, 2.5f);
        else
    ...
    private void Update()
    {
        ...
        if (data is AllyData)
            entityHealthBar.transform.localScale = new Vector2((float) data.BonesCurrent / (float) data.BonesMax * 2.5f, 0.3f);
        else
